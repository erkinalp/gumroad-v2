    ...
    <dl>
      ...
      <dd>
        {purchase.charge_transaction_url ? (
          <a href={purchase.charge_transaction_url} target="_blank" rel="noreferrer">
      ...
      purchase.card_visual &&
      purchase.card_visual.length > 0 ? (
        <>
          <dt>Card</dt>
          <dd>
      ...
      {purchase.is_preorder_authorization ? (
        <>
          ...
          <dd>
            <AdminPurchaseInfoEmailInfo email_infos={purchase.email_infos} email_name="preorder_receipt" />
          </dd>
        </>
      ) : (
        <>
          <dt>Receipt</dt>
          <dd>
            <AdminPurchaseInfoEmailInfo email_infos={purchase.email_infos} email_name="receipt" />
          </dd>
        </>
      )}
        ...
        <>
          ...
          <dd>
            ...
            {purchase.subscription.cancelled_at ? (
              <span>
                {" "}
                (on {purchase.subscription.cancelled_at} by{" "}
                {purchase.subscription.cancelled_by_buyer ? "buyer" : "seller"})
              </span>
            ) : null}
          </dd>
          ...
          <dd>
            ...
            {purchase.subscription.ended_at ? <span> (on {purchase.subscription.ended_at})</span> : null}
          </dd>

          <dt>Failed</dt>
          <dd>
            <BooleanIcon value={!!purchase.subscription.failed_at} />
            {purchase.subscription.failed_at ? <span> (on {purchase.subscription.failed_at})</span> : null}
          </dd>
        </>
      ) : null}
      ...
      {purchase.affiliate ? (
        <>
          <dt>Affiliate</dt>
          <dd>
            <Link href={Routes.admin_search_users_path({ query: purchase.affiliate.affiliate_user.form_email })}>
              {purchase.affiliate.affiliate_user.form_email}
            </Link>
          </dd>
        </>
      ) : null}
      ...
      {purchase.is_gift_sender_purchase ? (
        <>
          ...
          <dd>
            <Link href={Routes.admin_purchase_path(purchase.gift.giftee_purchase_id)}>
              {purchase.gift.giftee_purchase_id}
            </Link>
          </dd>
        </>
      ) : null}
      ...
      {purchase.is_gift_receiver_purchase ? (
        <>
          ...
          <dd>
            <Link href={Routes.admin_purchase_path(purchase.gift.gifter_purchase_id)}>
              {purchase.gift.gifter_purchase_id}
            </Link>
          </dd>
        </>
      ) : null}
